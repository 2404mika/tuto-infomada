...
class CustomTokenRefreshView(TokenRefreshView):
    # permission_classes = [IsAuthenticated]  # Permet à tous d'accéder pour rafraîchir
...
class FormationByUserPaymentAPIView(APIView):
    ...
    def post(self, request):
        try:
            # Récupérer les données du frontend avec débogage
            user_id = request.data.get('user_id')
            lastname = request.data.get('lastname')  # De EtudiantRegister
            ...
            # Vérifier si l'utilisateur existe et récupérer lastname depuis EtudiantRegister
            try:
                ...
                try:
                    ...
                    lastname = user.username  # Fallback
            except User.DoesNotExist:
                return Response({"error": "Utilisateur non trouvé"}, status=status.HTTP_404_NOT_FOUND)

            # Préparer les données pour Formation_by_user
            default_status_id = Status.objects.get(value=0).id  # Récupère l'ID de "Not Published"
            formation_by_user_data = {
                'user_id': user_id,
                'formation_id': formation_id,
                'student_name': lastname,
                'status': default_status_id  # Passer l'ID du statut
            }
            ...
            payment_data = {
                ...
                'paid_amount': price,
                'ref_transaction': ref_transaction,
                'destination_number': '0345929075',  # Valeur par défaut
                'status': 'PENDING',
            }
            ...
            return Response({
                ...
                'payment': PaymentSerializer(payment).data  # Assure que payment est renvoyé
            }, status=status.HTTP_201_CREATED)
...
class FormationByUserListAPIView(ListAPIView):
    ...
    def get_queryset(self):
        # Filtrer les inscriptions pour l'utilisateur connecté et précharger les paiements
...
class AdminFormationByUserApproveAPIView(APIView):
    ...
    def put(self, request, pk, *args, **kwargs):
        try:
            ...
            published_status = Status.objects.get(value=1)  # Published
...
class AdminFormationByUserListAPIView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        try:
            # Récupérer tous les formation_by_user avec les relations préchargées
...
class VideoAccessView(APIView):
    ...
    def get(self, request, formation_by_user_id):
        try:
            ...
            # Logique pour retourner les vidéos (à implémenter selon ton modèle Video)
...
class PaymentReceiptView(APIView):
    ...
    def get(self, request, payment_id):
        ...
        response['Content-Disposition'] = f'attachment; filename="receipt_payment_{payment_id}.pdf"'
        
        p = canvas.Canvas(response)
        p.drawString(100, 750, f"Reçu de Paiement")
        p.drawString(100, 730, f"ID de Paiement: {payment.id}")
