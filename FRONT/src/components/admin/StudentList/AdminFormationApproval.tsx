...
const AdminFormationApproval: React.FC = () => {
  ...
  const [filteredFormations, setFilteredFormations] = useState<FormationByUser[]>([]);
  ...
  useEffect(() => {
    const filtered = formations.filter(formation =>
      formation.student_name.toLowerCase().includes(searchQuery.toLowerCase()) ||
      formation.formation_title.toLowerCase().includes(searchQuery.toLowerCase()) ||
      (formation.payment?.ref_transaction?.toLowerCase().includes(searchQuery.toLowerCase()) ?? false)
    );
  ...
  return (
    <div className="container mx-auto p-4 max-w-3xl">
      <div className="flex items-center justify-between mb-4 flex-wrap">
        <h2 className="text-2xl font-semibold text-gray-800">Approbation des demandes de formations</h2>
        <input
          type="text"
          placeholder="Référence ou Nom ou Titre...."
          value={searchQuery}
        ...
        <div className="space-y-4">
          {[...filteredFormations]
            .sort((a, b) => {
              ...
              return isBNotPublished - isANotPublished;
            })
            .map((formation) => {
