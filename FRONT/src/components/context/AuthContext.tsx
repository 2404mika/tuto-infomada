...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const login = (userData: User, accessToken: string, refreshToken: string) => {
    ...
    // Pas d'appel à refreshUser ici, car user est déjà mis à jour
  };
    ...
    try {
      ...
      try {
        ...
        if (refreshResponse.ok) {
          ...
          await refreshUser(); // Relance avec le nouveau token
        } else {
...
//     }
//   }, []); // Dépendance vide, ne se relance pas après login
...
//     // Attend que les nouveaux tokens soient propagés dans l'état avant d'appeler refreshUser
//     await new Promise((resolve) => setTimeout(resolve, 0)); // Force une mise à jour asynchrone
//     await refreshUser().catch((error) => console.error('Erreur post-login:', error));
//   };
...
//         const data = await response.json();
//         console.log('RefreshUser data:', data);
//         setUser(data); // Met à jour user avec les données complètes
//         localStorage.setItem('user', JSON.stringify(data));
//       } else {
//         throw new Error('Erreur lors de la récupération des données utilisateur: ' + await response.text());
//       }
